...
namespace AnEmailService.Nini.Config
{
    ...
    public class XmlConfigSource : ConfigSourceBase
    {
        ...
        private void RemoveSections()
        {
            ...
            foreach (XmlNode node in configDoc.DocumentElement.ChildNodes)
                    ...
                    && node.Name == "Section")
                {
                    ...
                    if (attr != null)
                    {
                        if (Configs[attr.Value] == null) configDoc.DocumentElement.RemoveChild(node);
                    }
                    else
                    {
            ...
            if (sectionNode != null)
                foreach (XmlNode node in sectionNode.ChildNodes)
                        ...
                        && node.Name == "Key")
                    {
                        ...
                        if (keyName != null)
                        {
                            if (Configs[sectionName].Get(keyName.Value) == null) sectionNode.RemoveChild(node);
                        }
                        else
                        {
        ...
        private void PerformLoad(XmlDocument document)
        {
            ...
            if (document.DocumentElement.Name != "Nini") throw new ArgumentException("Did not find Nini XML root node");
        ...
        private void SetKey(XmlNode sectionNode, string key, string value)
        {
            ...
            if (node == null) CreateKey(sectionNode, key, value);
            else node.Attributes["Value"].Value = value;
        }
        ...
        private void RemoveConfigs()
        {
            ...
            for (int i = Configs.Count - 1; i > -1; i--)
            {
                ...
                if (GetSectionByName(config.Name) == null) Configs.Remove(config);
            }
        }
        ...
        private void RemoveConfigKeys(IConfig config)
        {
            ...
            foreach (string configKey in configKeys)
                if (GetKeyByName(section, configKey) == null) config.Remove(configKey);
